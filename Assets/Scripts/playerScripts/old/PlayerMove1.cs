...
public class PlayerMove1 : MonoBehaviour
{
    ...
    public float JumpFallingGravityMultiplier;
    public float JumpPeakGravityMultiplier;
    public float MinGravityYVelocity, MaxGravityYVelocity;
    ...
    void Timers()
    {
        ...
        //Gliding
        {
            if ((GroundedTime > 0 || isWallRunning) && GlideLength <= GlideLengthMax)
            {
                GlideLength += GlideChargeMultiplier * Time.deltaTime;
                GlideCharge = true;
            } else if (GlideCharge && GlideLength <= GlideLengthMax)
            {
                GlideLength += GlideChargeMultiplier * Time.deltaTime;
            }
